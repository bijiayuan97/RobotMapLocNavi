...
def a_star(start_index, goal_index, width, height, costmap, resolution, origin, grid_viz):
  ...
  # dict for mapping f costs (h cost + g cost) to nodes
  f_costs = dict()
  g_costs = dict()
  ...
  while open_list:
    ...
    for neighbor_index, step_cost in neighbors:
      ...
      # determine the g cost for the current neighbor
      g_cost = g_costs[current_node] + step_cost

      # determine the f cost for the current neighbor
